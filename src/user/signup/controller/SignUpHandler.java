...
//		 return null;
	...
	private static SingnUpRequest mappingObject(HttpServletRequest request, HttpServletResponse response) {
		return new SingnUpRequest(request.getParameter("email"), request.getParameter("name"),
				request.getParameter("id"), request.getParameter("password"), request.getParameter("confirmPassword"),
				checkAgree(request));
	}
